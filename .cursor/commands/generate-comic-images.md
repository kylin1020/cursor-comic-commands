...
- 读取 `style.md` 获取全局风格
- 读取相关角色/场景/分镜的md文件获取提示词
- 检查 outputs 目录中已存在的参考图
...
- 准备参考图（最多6张）：
  - 角色衍生图：使用正面照作为参考
  - 场景衍生图：使用远景图作为参考
  - 分镜：使用角色正面照 + 场景远景 + (可选)前一分镜尾帧

### 步骤3：按顺序并发生成
**第一轮**：生成基础参考图（正面照、远景图）- 不使用参考图
**第二轮**：生成衍生图（三视图、表情、动作、中景、近景等）- 使用基础参考图
**第三轮**：生成分镜（最多3个并发任务，即6张图）- 使用角色和场景参考图
...
1. 分镜中出现的角色的正面照
2. 分镜对应场景的远景图
3. (可选) 前一分镜的尾帧 - 如果有连续性（场景相同、角色动作延续、镜头推拉）
