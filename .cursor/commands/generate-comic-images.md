...
- [ ] **分镜特殊处理**：合并前帧和尾帧提示词，开头明确要求"画两张图片。第一张图：...。第二张图：..."，设置num_images=2，一次性生成两张关联图片

### 阶段4：批量生成图片
- [ ] 确认输出目录（不存在则创建）
- [ ] 第一轮：并发生成所有基础参考图（不依赖其他图片的正面照、远景等）
- [ ] 第二轮：并发生成所有使用已存在参考图的衍生图片（三视图、表情、动作、中景、近景等）
- [ ] 第三轮：如有分镜，并发生成分镜图（每个分镜使用num_images=2一次生成前帧和尾帧，使用已有角色和场景参考图，一次最多并发3个分镜任务即6张图）
...
为每个需要生成的图片准备以下内容：
- **提示词**: 从md文件中提取完整的AI生成提示词（不要追加额外要求）
  - **分镜特殊处理**：合并提示词时，开头明确说明"画两张图片。第一张图：<前帧提示词>。第二张图：<尾帧提示词>"，设置 `num_images=2`
- **参考图**: 用于保持一致性的参考图输入
  - 角色三视图/表情/动作图：使用该角色的正面照作为参考
  - 场景中景/近景：使用该场景的远景图作为参考
  - 分镜：使用对应角色的正面照和场景图作为参考
  - **分镜连续性**：根据分镜内容，如果与前一个分镜有连续性，将前一个分镜的尾帧也加入参考图
  - 基础参考图（正面照、远景）：不使用参考图，纯文本生成
- **输出路径**: 输出目录和文件名（文件名传入filename参数，编号会自动生成）
...
- **分镜前帧和尾帧关联生成** - 每个分镜的前帧和尾帧是关联的两张图，使用 `num_images=2` 一次性生成，确保两帧之间的视觉连贯性
...
- 🎬 **分镜优化**：
  - 前帧和尾帧必须关联生成（num_images=2），提高两帧之间的连贯性
  - 根据分镜内容判断是否需要使用前一分镜的尾帧作为参考图，增强分镜间的连续性
...
分镜的前帧和尾帧是同一镜头的开始和结束画面，应当一起生成（`num_images=2`）以保证：
- ✅ 视觉连贯性：角色外貌、场景风格、光线氛围一致
- ✅ 动作流畅性：前帧到尾帧的动作过渡自然
- ✅ 时空一致性：同一时间段的画面变化
...
```
{
  "prompt": "画两张图片。第一张图：<前帧提示词>。第二张图：<尾帧提示词>",
  "num_images": 2,
  "reference_images": [角色正面照, 场景远景, (可选)前一分镜的尾帧],
  "guidance_scale": 2.5,
  "size": "2K",
  "output_directory": "outputs/chapters/",
  "filename": "scene_001"  // 自动生成 scene_001_1.png, scene_001_2.png
}
```

### 提示词处理

- 提示词开头要明确要求生成两张图：**"画两张图片，第一张图..."，"第二张图..."**
- 从 `image-prompts.md` 提取前帧和尾帧提示词
- 合并格式示例：
  ```
...
分镜生成的参考图应包括：
1. **角色参考图**：分镜中出现的角色的正面照
2. **场景参考图**：分镜对应场景的远景图
3. **前一分镜的尾帧**（可选）：如果当前分镜与前一个分镜有连续性（如同一场景、同一角色动作延续），将前一个分镜的尾帧加入参考图，增强画面连贯性

**连续性判断**：
- 场景相同或相似 → 使用前一分镜尾帧
- 角色动作有延续 → 使用前一分镜尾帧
- 镜头有推拉关系 → 使用前一分镜尾帧
- 完全不同的场景/角色 → 不使用前一分镜尾帧

### 批量生成策略

- 一次最多并发 **3个分镜任务**（即6张图片）
- 每个任务使用 `num_images=2` 生成前帧和尾帧
- 使用 `batch_generate_images` 工具进行批量生成
- 按顺序生成，以便后续分镜可以使用前一分镜的尾帧作为参考

### 质量检查重点

- ✅ 前帧和尾帧的角色外貌、场景风格是否一致
- ✅ 动作过渡是否自然，光线氛围是否统一
- ✅ 分镜间的连续性（如果使用了前一分镜尾帧作为参考）

### 失败重试策略

- **局部问题**：将成功的那一帧加入 `reference_images`，重新生成整个分镜（num_images=2）
- **整体问题**：检查参考图是否正确，调整后重新生成
- **连续性问题**：如果分镜间不连贯，检查是否正确使用了前一分镜的尾帧

### 生成建议

- 按顺序分批生成，每批3-5个分镜
- 第一批生成后检查效果，确认参考图和提示词无误
- 后续分镜可以使用前面已生成的分镜尾帧作为参考
